...
| 11 | [x] | CartItem decrement button has no `disabled` prop at `quantity === 1` — sends `quantity: 0` to API which rejects with 400; button appears clickable but errors silently | `apps/web/src/components/CartItem.tsx` | 41 |
| 12 | [x] | Cart count in navbar uses `queryKey: ["cart-count"]` but cart mutations invalidate `["cart"]` — nav badge is perpetually stale after any cart change | `apps/web/src/routes/__root.tsx` | 16 |
